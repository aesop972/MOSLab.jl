...
function PSP_psiSp(Vg,Na,tox,Vfb,T,φb,Vsb=0.0)
    phit = kb*T
    φb *= 2.0
    φb = abs(φb)
    phib = φb
...
function PSP_psid(Vg,Na,tox,Vfb,T,φb;Vsb=0.0,Vdse=0.0)
    ...
    ψs,Es,Ds,Ps,xgs = PSP_psiSp(Vg,Na,tox,Vfb,T,φb,Vsb)
    xs = ψs*phit
    xds = xd-xs
    if  xds < 1e-10
...
function PSP_VaricapPsip(Vg,Na,tox,Vfb,T,φb)
    #psisd,Ed,Dd,Δψ = PSP_psid(Vg,Na,tox,Vfb,T,φb)
    psiss,Es,Ds,Ps,xgs = PSP_psiSp(Vg,Na,tox,Vfb,T,φb) 
    return psiss
end


function PSP_ψm(Vg,Vd,Vs,Na,tox,Vfb,T,φb)
    psiss,Es,Ds,Ps,xgs = PSP_psiSp(Vg,Na,tox,Vfb,T,φb,Vs)
    psisd,Ed,Dd,Pd,xds = PSP_psiSp(Vg,Na,tox,Vfb,T,φb,Vd)
    phit = kb*T
    φb *= 2.0
    G = γ(Na,tox)/sqrt(phit)
    x_s = psiss/phit
...
function PSP_Gvsat(Vg,Vd,Vs,Na,tox,Vfb,T,φb,Cox,CFD,CF)
    ...
    Vg = Vg+ΔVgb
    psiss,Es,Ds,Ps,xgs = PSP_psiSp(Vg,Na,tox,Vfb,T,φb,Vs)
    psisd,Ed,Dd,Pd,xds = PSP_psiSp(Vg,Na,tox,Vfb,T,φb,Vdseff)
    xds = (psisd-psiss)
    phit = kb*T
    G = γ(Na,tox)/sqrt(phit)
    qis =  PSP_qis(G,Ds,xgs,Ps,phit)
